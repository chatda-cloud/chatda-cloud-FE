...
- **분실물 탐색 (Search)**
  ...
  - 분실물/습득물 아이템 카드 내 **타임스탬프** 및 상세 정보 노출.
...
- **사용자 마이페이지 (My Page)**
  - 프로필 헤더의 수정 아이콘(✏️)으로 내 정보(이름, 전화번호 등) 수정 진입.
  - 탭 구성: **분실물 / 습득물 / 매칭** 3탭으로 내 물건 통합 관리.
  - **설정 화면** (알림 설정, 로그아웃, 회원 탈퇴).
- **인증 (Auth)**
  - 로그인 및 회원가입 통합 화면.
...
프론트엔드의 화면들은 크게 `lib/features/` 내에 역할별로 깔끔하게 분리되어 있습니다.
...
│   ├── auth/           # 로그인 / 회원가입 (자동 로그인)
│   ├── chat/           # 채팅 목록 / 1:1 대화 상세
│   ├── home/           # 홈 대시보드 (매칭 추천 현황)
│   ├── main/           # 하단 5탭 네비게이션 메인 라우터
│   ├── match/          # 유사도 상세 분석 메뉴
│   ├── mypage/         # 마이페이지, 설정, 알림 설정
...
- **Target OS**: iOS (노치/다이나믹 아일랜드 대응), Android (펀치홀 대응 Safe Area 지원)
- **Design System**: Material Design 3 (Customization 적용)
